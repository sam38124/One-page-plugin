                        ...
                        view: () => {
                            ...
                            return `
                                ...
                                menu.tagObject.tags.map((t, i) => {
                                    tmp += ` <li data-filter=".${t.className}" ${i == 0 ? `class="filter-active"` : ``} onclick="${gvc.event(() => {
                                        document.querySelector('#' + gvc.id(id)), function (instance) {
                                            $(".menu-container").isotope({ filter: ':not("*")' }), $(".menu-container").isotope({ filter: "*" });
                                        };
                                    })}">${t.title}</li> `;
                                });
                                return tmp;

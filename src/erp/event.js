            ...
            return {
                ...
                event: () => {
                    return new Promise((resolve, reject) => {
                        ...
                        ErpConfig.roleList = object.childAccount.map((dd) => {
            ...
            return {
                editor: () => {
                    return Editor.arrayItem({
                        ...
                        gvc: gvc,
                        title: '設定用戶來源',
                        array: object.comeFromArray.map((dd, index) => {
                ...
                event: () => {
                    const shareDialog = new ShareDialog(gvc.glitter);
                    BaseApi.create({
                        "url": ErpConfig.api + `/api/v1/user/getMember?role=${object.comeFromArray.map((dd) => {
                            return dd.comeFrom;
                        }).join(',')}`,
                        "type": "GET",
                        "timeout": 0,
        ...
        fun: (gvc, widget, object, subData) => {
            return {
                ...
                event: () => {
                    ...
                    if (index !== -1) {
                        subData.deadLine = window.moment().add([14, 30, 60][index], "days").format('YYYY-MM-DD HH:mm:ss');
                    }
                    BaseApi.create({
                ...
                event: () => {
                    ...
                    BaseApi.create({
                        ...
                        if (d2.result) {
                            ...
                            d2.response.data = d2.response.data.filter((dd) => {
                                            ...
                                            return {
                                                ...
                                                onCreate: () => {
                                                    gvc.glitter.addMtScript([
                                                        ...
                                                        new window.QRCode(document.getElementById(gvc.id(visible)), {
                                                            text: url.href,
                                                            width: 512,
                                                            height: 512,
                                                        });
                                                        $(`#${gvc.id(id)} img`).click((event) => {
                                                            glitter.openDiaLog(new URL('../dialog/image-preview.js', import.meta.url), 'preview', $(`#${gvc.id(visible)} img`).attr('src'));
                                                            event.stopPropagation();
                                                        });
                                                    }, () => {
                                                    });
                                    ...
                                    },
                                    {
                                        key: '[出貨/拆櫃]', value: (() => {
                                            ...
                                            if (index !== -1) {
                                                ...
                                                const endDate = window.moment(dd.config.deadLine).format('YYYY-MM-DD');
                                                const min = window.moment(endDate).diff(startDate, 'day');
                                                if (min > 0) {
                                            ...
                                            if (['waitOut', 'needCheck'].indexOf(dd.config.order_status) !== -1) {
                                                ...
                                                if (dd.config.order_status === 'to-shipp') {
                                                    const startDate = window.moment();
                                                    const endDate = window.moment(dd.config.unpack_date).format('YYYY-MM-DD');
                                                    const min = window.moment(endDate).diff(startDate, 'day');
                                                    if (min >= 0) {
                                                        return `<span style="color:orange;">剩餘${min}天</span>`;
                                                    }
                            ...
                            subData.editData = d2.response.data.filter((dd) => {
                                try {
                                    dd.config.id = dd.id;
                                    if (dd.product) {
                            ...
                            subData.editData = subData.editData.map((dd) => {
            ...
            return {
                ...
                event: () => {
                    ...
                    if (subData.order_status === 'Shipped') {
                        ...
                        if (!subData['package_list'] || subData['package_list'].find((dd) => {

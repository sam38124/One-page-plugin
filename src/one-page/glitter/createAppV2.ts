...
const id2=glitter.getUUID()
 ...
 >${template.title}</h2>
          <p class="fs-lg pb-2 pb-md-3 pb-lg-0 mb-4 mb-lg-5  ${glitter.htmlGenerate.styleEditor(widget.data.descStyle).class()}"
            ...
            >${template.desc}</p>
            <div class="row">
               <ul class="nav nav-tabs flex-nowrap  overflow-auto  flex-sm-column col-sm-2 col-12 m-0 ps-2 p-2" role="tablist">
            ...
            ${glitter.print(function () {
                            return template.list.map((p: any) => {
                                ...
                                if((filter!=="*")&&(tagClass.indexOf(filter)===-1)){
                                    return  ``
                                }
                                return `
                  <div class=" masonry-grid-item col-md-4 col-sm-6 col-12 ${tagClass} mb-4 " >
        ...
        editor: () => {
            ...
            return gvc.map([
                ...
                Editor.arrayItem({
                    ...
                    gvc: gvc,
                    title: '標籤設定',
                    array: widget.data.tag.map((data: any, index: number) => {
                        return {

                            ...
                            Editor.toggleExpand({
                                ...
                                innerText: (() => {
                                    function recursion(data, first) {
                                        ...
                                        const innerHtml = data.list.map((dd, index) => {
...
${widget.data.list.map((dd, index) => {
                                return Editor.toggleExpand({
                                    gvc: gvc, title: dd.name || `項目:${index + 1}`, data: dd, innerText: gvc.map([
                                        Editor.fontawesome({
                                            gvc: gvc, title: "圖示", def: dd.icon, callback: (text) => {
                                                dd.icon = text;
                                                widget.refreshComponent();
                                            }
                                        }),
                ...
                `}
                      ...
                      <button
                        ...
                        onclick="${gvc.event(function () {
                                ...
                                glitter.openNewTab(`mailto:jianzhi.wang@ncdesign.info?body=${encodeURIComponent(data.map((dd) => {
                                    return `${dd.name}:\n${dd.value}`;
                                }).join(`\n\n`))}`);
                            })}"
                      >

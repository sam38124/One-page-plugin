            ...
            return {
                view: () => {
                    ...
                    return `<div class="w-100 p-0 m-0" style="white-space:normal;">${subData.formData.log.filter((dd: any) => {
                        ...
                        function updateText(data: any, formList: any): any {
                            const key = formList.find((dd: any) => {
                            ...
                            if (key.type === 'select') {
                                if (data.selectType === 'api') {
                                    ...
                                    try {
                                        const dd = key.selectList.find((dd: any) => {
                                            return dd.value === data.value
                                        }) ?? {name: data.value}
...
${`<span style="" class="fw-bold text-warning" >${(() => {
                            const key = subData.formList.find((dd: any) => {
                                return dd.key === data.key
                            })
                            if (key.group) {
